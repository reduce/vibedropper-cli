	...
	Command = &cli.Command{
		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				UsageText:       "vibedropper @manpages [-o vibedropper.1] [--gzip]",
				Hidden:          true,
...
func generateManpages(ctx context.Context, c *cli.Command) error {
	...
	if c.Bool("text") {
		file, err := os.Create(filepath.Join(dir, "man1", "vibedropper.1"))
		if err != nil {
			return err
		}
		defer file.Close()
	...
	if c.Bool("gzip") {
		file, err := os.Create(filepath.Join(dir, "man1", "vibedropper.1.gz"))
		if err != nil {
			return err
		}
		defer file.Close()
